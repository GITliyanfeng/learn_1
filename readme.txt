...
同一个分支,两个客户端同时对同一个文件的文件名做了修改(没有修改文件内容),其中一个先提交,那么最后一个无法提交到远端,这时候需要pull,pull的结果是旧文件,当前客户端更改后的文件,其他客户端更改名字后的文件同时处于工作区中,需要我们手动的去git
rm掉不需要的文件git
add保留最终确定的文件,然后commit,最后push,这样才能提交到远端 

协同开发的过程中 git push -f 强制推送是禁止使用的,会丢失远端仓库的编辑记录
